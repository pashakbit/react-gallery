...
class Gallery extends React.Component{
	...
	var lightboxImages = [];
		...
		if (this.props.disableLightbox){
		    photoPreviewNodes.push(
			 <div key={k} className={this.props.childClass}>
			    <img src={src} alt="" />
			    <button key={k} onClick={this.props.removePhoto.bind(this, this.props.groupId, this.props.photos[k].src)} className={this.props.deleteBtnClass}>{String.fromCharCode(215)}</button>
			 </div>
		    );
		}
		else{
	...
	if (this.props.disableLightbox){
	    ...
	    return(
		<div id="Gallery" className={this.props.className} ref={(c) => this._gallery = c}>
		    {photoPreviewNodes}
		    <Lightbox
